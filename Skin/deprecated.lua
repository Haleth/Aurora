...
F.CreateGradient = function(f)
    ...
    if f.GetBackdropTexture then
        if _G.AuroraConfig.buttonsHaveGradient then
            ...
            f:SetBackdropColor(private.buttonColor:GetRGBA())
        end
    else
        ...
        tex:SetVertexColor(private.buttonColor:GetRGBA())
    end
    return tex
end
...
F.Reskin = function(f, noHighlight)
    ...
    Base.SetBackdrop(f, private.buttonColor:GetRGBA())
...
F.ReskinTab = function(f, numTabs)
    if numTabs then
        ...
        local red, green, blue = private.highlightColor:GetRGB()
...
F.ReskinScroll = function(f, parent)
    ...
    Base.SetBackdrop(bu.bg, private.buttonColor:GetRGBA())
...
F.ReskinInput = function(f, height, width)
    ...
    Base.SetBackdrop(bd, private.frameColor:GetRGBA())
